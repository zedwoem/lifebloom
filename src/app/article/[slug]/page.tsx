...
5. Ensure the tone is warm, extremely accessible, and authoritative. Do not wrap in markdown blocks, html, head, or body tags — output only the clean inner HTML.`;
  ...
  const dateStr = new Date('2026-05-20T08:00:00Z').toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
  ...
  if (geminiKey) {
    try {
      ...
      const prompt = `You are a professional, expert editor at LifeBloom Hub. Write a highly informative, detailed, 600-word professional article in ${'English'} for senior citizens about the topic: "${decodedSlug}". 

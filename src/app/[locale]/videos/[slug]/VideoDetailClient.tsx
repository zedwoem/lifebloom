...
export default function VideoDetailClient({ video, relatedVideos, locale }: VideoDetailClientProps) {
    ...
    try {
      if (isBookmarked) {
        ...
        if (res.ok) {
          ...
          toast.success(locale === "id" ? "Video dihapus dari simpanan." : "Video removed from saved items.");
        }
      } else {
        const res = await fetch("/api/user/saved-items", {
          method: "POST",
        ...
        if (res.ok) {
          ...
          toast.success(locale === "id" ? "Video berhasil disimpan!" : "Video saved to your dashboard!");
        } else {
          toast.error(locale === "id" ? "Silakan masuk untuk menyimpan video." : "Please sign in to save videos.");
        }
      }
    } catch (err) {
  ...
  const copyToClipboard = (text: string, isEmbed: boolean) => {
    ...
    if (isEmbed) {
      setCopiedEmbed(true);
      toast.success(locale === "id" ? "Kode sematan disalin!" : "Embed code copied to clipboard!");
      setTimeout(() => setCopiedEmbed(false), 2000);
    } else {
      setCopiedLink(true);
      toast.success(locale === "id" ? "Link video disalin!" : "Video link copied to clipboard!");

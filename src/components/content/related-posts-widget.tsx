  ...
  return (
    <div className="mt-16 pt-8 border-t border-slate-200 print:hidden select-none">
      ...
      <div className="mb-12 p-6 bg-emerald-50 rounded-2xl border border-emerald-100">
        ...
        <ul className="space-y-3">
          {newAndPopular.slice(0, 2).map((post, idx) => (
            <li key={idx}>
              <Link href={`/article/${post.slug}`} className="text-emerald-700 hover:text-emerald-900 font-medium hover:underline flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-12">
        <Link href={`/article/${previousPost.slug}`} className="group p-6 rounded-2xl border border-slate-200 hover:border-brand-green/50 hover:bg-slate-50 transition-all flex flex-col items-start text-left">
        ...
        <Link href={`/article/${nextPost.slug}`} className="group p-6 rounded-2xl border border-slate-200 hover:border-brand-green/50 hover:bg-slate-50 transition-all flex flex-col items-end text-right">
          <span className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-1 group-hover:text-brand-green">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-slate-100">
          {newAndPopular.map((post, idx) => (
            <Link key={idx} href={`/article/${post.slug}`} className="block p-4 hover:bg-slate-50 transition-colors group">
